...
Function .onInit
  ...
  ${If} $INSTDIR == "${PLACEHOLDER_INSTALL_DIR}"
    ...
    !else if "${INSTALLMODE}" == "currentUser"
      ; OwLLM customization: install under \Programs\ (Chrome / VS Code
      ; / Slack convention) so the binary directory doesn't collide
      ; with the data folder the running app uses at
      ;   %LOCALAPPDATA%\${PRODUCTNAME}\
      ; (downloaded modules, runtime, settings).
