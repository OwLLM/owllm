...
fn curated() -> Vec<Seed> {
    vec![
        ...
        Seed::new("meta-llama/Llama-3.2-1B-Instruct", "Llama 3.2 1B Instruct", "Llama", 1.2, "Tiny Llama for edge / CPU use. Best for fast iteration.", "instruct", 850_000, 6_400, true, true),
        Seed::new("meta-llama/Llama-3.2-3B-Instruct", "Llama 3.2 3B Instruct", "Llama", 3.2, "Mid-small Llama. Sweet spot for laptops with 8 GB GPUs.", "instruct", 1_400_000, 10_200, true, true),
        Seed::new("Qwen/Qwen2.5-1.5B-Instruct", "Qwen 2.5 1.5B Instruct", "Qwen", 1.5, "Compact Qwen — strong multilingual for its size.", "instruct", 320_000, 2_700, true, false),
        Seed::new("google/gemma-2-2b-it", "Gemma 2 2B IT", "Gemma", 2.6, "Google's small instruction-tuned model.", "instruct", 580_000, 5_100, false, true),
        Seed::new("microsoft/Phi-3.5-mini-instruct", "Phi 3.5 mini", "Phi", 3.8, "MS Phi 3.5 — punches above its weight on reasoning.", "reasoning", 420_000, 3_800, true, false),

        // ---- Mid (fits on 16–24 GB cards) ----
        Seed::new("meta-llama/Llama-3.1-8B-Instruct", "Llama 3.1 8B Instruct", "Llama", 8.0, "The default chat workhorse. Strong on general tasks.", "instruct", 4_200_000, 28_400, false, true),
        Seed::new("Qwen/Qwen2.5-7B-Instruct", "Qwen 2.5 7B Instruct", "Qwen", 7.6, "Best mid-size model for multilingual + code tasks.", "instruct", 1_900_000, 14_300, true, false),
        Seed::new("mistralai/Mistral-7B-Instruct-v0.3", "Mistral 7B Instruct v0.3", "Mistral", 7.3, "Mistral's reliable 7B instruct — Apache-2.0 friendly.", "instruct", 2_800_000, 18_700, false, false),
        Seed::new("google/gemma-2-9b-it", "Gemma 2 9B IT", "Gemma", 9.2, "Google's mid-tier instruct model. Best Gemma for chat.", "chat", 760_000, 7_400, false, true),
        Seed::new("01-ai/Yi-1.5-9B-Chat", "Yi 1.5 9B Chat", "Yi", 8.8, "Yi chat — strong English + Chinese performance.", "chat", 110_000, 1_800, false, false),
        Seed::new("deepseek-ai/deepseek-coder-6.7b-instruct", "DeepSeek Coder 6.7B", "DeepSeek", 6.7, "Code-tuned base model. Use for repo-level coding tasks.", "code", 540_000, 4_300, false, false),
        Seed::new("Qwen/QwQ-32B-Preview", "QwQ 32B Preview", "Qwen", 32.5, "Qwen's reasoning preview — chain-of-thought tuned.", "reasoning", 420_000, 7_800, true, false),
        Seed::new("microsoft/Phi-3-medium-4k-instruct", "Phi 3 Medium 4k", "Phi", 14.0, "MS Phi 3 medium — 4k context, instruction-tuned.", "instruct", 280_000, 2_200, false, false),

        // ---- Large (fits on 48 GB+ cards or with QLoRA on 24 GB) ----
        Seed::new("mistralai/Mistral-Nemo-Instruct-2407", "Mistral Nemo 12B Instruct", "Mistral", 12.2, "Mistral + NVIDIA collab. 128k context, strong instruct.", "instruct", 1_200_000, 9_500, false, false),
        Seed::new("Qwen/Qwen2.5-32B-Instruct", "Qwen 2.5 32B Instruct", "Qwen", 32.5, "Larger Qwen — frontier-level reasoning on consumer 48 GB.", "instruct", 680_000, 5_300, true, false),
        Seed::new("deepseek-ai/DeepSeek-V2-Lite", "DeepSeek V2 Lite", "DeepSeek", 15.7, "DeepSeek MoE — small active params, huge expert pool.", "instruct", 220_000, 1_900, false, false),
        Seed::new("meta-llama/Llama-3.1-70B-Instruct", "Llama 3.1 70B Instruct", "Llama", 70.0, "Frontier-tier chat. Needs 2×A100 or QLoRA on 48 GB.", "chat", 2_100_000, 18_200, false, true),
        Seed::new("Qwen/Qwen2.5-72B-Instruct", "Qwen 2.5 72B Instruct", "Qwen", 72.7, "Top-tier Qwen. Same hardware ask as Llama 70B.", "chat", 480_000, 4_900, true, false),
        Seed::new("mistralai/Mixtral-8x7B-Instruct-v0.1", "Mixtral 8×7B Instruct", "Mistral", 46.7, "Mixtral MoE — ~12B active params per token.", "instruct", 1_400_000, 12_400, false, false),

        // ---- Vision ----
        Seed::new("Qwen/Qwen2-VL-7B-Instruct", "Qwen2-VL 7B", "Qwen", 8.3, "Vision-language Qwen. Image + text in, text out.", "vision", 540_000, 4_900, true, false),
    ]
}
...
impl Seed {
    fn new(
        id: &'static str, name: &'static str, family: &'static str,
        params_b: f32, description: &'static str, category: &'static str,
        downloads: u64, likes: u64, is_new: bool, gated: bool,
    ) -> Self {
        Self { id, name, family, params_b, description, category, downloads, likes, is_new, gated }
    }
}
...
fn tags_for(seed: &Seed) -> Vec<String> {
    let mut t = Vec::new();
    match seed.category {
        "instruct"  => { t.push("instruct".into()); }
        "chat"      => { t.push("chat".into()); }
        "reasoning" => { t.push("reasoning".into()); t.push("instruct".into()); }
        "vision"    => { t.push("vision".into()); }
        "code"      => { t.push("code".into()); t.push("instruct".into()); }
        _ => {}
    }
