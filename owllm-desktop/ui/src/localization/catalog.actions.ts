...
export const ACTION_CATALOG: string[][] = [
  ...
  ["Coder is mid-run — the fix task is queued as a ⚡ steer and will run next.", "编码智能体正在运行——修复任务已加入 ⚡ 引导队列，稍后将执行。", "코더가 실행 중입니다 — 수정 작업이 ⚡ 스티어로 대기열에 추가되어 곧 실행됩니다.", "コーダーは実行中です — 修正タスクは ⚡ ステアとしてキューに入り、次に実行されます。", "الوكيل قيد التشغيل — تمت جدولة مهمة الإصلاح كتوجيه ⚡ وستُنفذ تاليًا.", "Il coder è in esecuzione — il task di correzione è in coda come ⚡ steer e partirà a breve."],
  ["Can't start the fix — no model is selected in the Coder pane. Pick a model above the chat, then press 🛠 Fix with agent again.", "无法开始修复——编码窗格未选择模型。请在聊天上方选择一个模型，然后再次按 🛠 让智能体修复。", "수정을 시작할 수 없습니다 — 코더 창에 모델이 선택되어 있지 않습니다. 채팅 위에서 모델을 선택한 후 🛠 에이전트로 수정을 다시 누르세요.", "修正を開始できません — コーダーペインでモデルが選択されていません。チャット上部でモデルを選び、もう一度 🛠 エージェントで修正を押してください。", "تعذر بدء الإصلاح — لم يتم اختيار نموذج في لوحة المبرمج. اختر نموذجًا أعلى الدردشة ثم اضغط 🛠 إصلاح بواسطة الوكيل مرة أخرى.", "Impossibile avviare la correzione — nessun modello selezionato nel pannello Coder. Scegli un modello sopra la chat, poi premi di nuovo 🛠 Correggi con l'agente."],
  ["Can't start the fix — no workspace is connected on this page.", "无法开始修复——此页面未连接工作区。", "수정을 시작할 수 없습니다 — 이 페이지에 연결된 워크스페이스가 없습니다.", "修正を開始できません — このページにワークスペースが接続されていません。", "تعذر بدء الإصلاح — لا توجد مساحة عمل متصلة بهذه الصفحة.", "Impossibile avviare la correzione — nessuna area di lavoro collegata a questa pagina."],
];
