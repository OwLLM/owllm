...
const PAGE_DOCS: Record<string, string> = {
  ...
  agents:
    ...
    "• Notebook (📓 on the canvas header): your scratchpad DURING a run — one Working notes pane, a 📋 PLAN pane, a NEXT-STEPS list, and a 🪄 Digest notes action that reads the notes/current plan/existing steps and proposes a plan update + clear new steps (ADDITIVE — it only proposes, you apply/add what you like). ⚡ Feed sends a step to the running team as a steer, or dispatches it as a new goal when idle; Auto-feed dispatches the next pending step after each clean finish. Saved per project.\n" +
  ...
  code:
    ...
    "• 📓 NOTEBOOK tab: Working notes, a 📋 PLAN pane, a NEXT-STEPS list, and 🪄 Digest notes — pick its model right there (the same dropdown as everywhere; ✕ = back to the default). Digest reads the notebook once and proposes a plan update + new steps; there is no second prompt box. Feeding a step while the coder works = a ⚡ steer (injected between tool calls on local models); idle = a new turn. 'Auto-feed next step' walks the list by itself after each clean finish.\n" +
