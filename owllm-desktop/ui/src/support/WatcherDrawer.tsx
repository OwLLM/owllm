...
const PAGE_DOCS: Record<string, string> = {
  ...
  devices:
    ...
    "• DISCOVERY IS AUTOMATIC: devices signed into the same GitHub vault appear in 'My OwLLM Devices' with their addresses, published at every app launch and the moment remote control is toggled. A device showing 'no address yet' just hasn't had remote control enabled on it. Pair also re-pulls the vault before giving up, and 'Pair by IP' works with no vault at all.\n" +
    ...
    "• Transports: loopback (self-test), LAN/overlay direct, and relay — all carrying the same sealed frames.\n" +
    "• INTERACTIVE SHELL (SSH-like): the '🖥 Open shell' button in the remote console opens a REAL live terminal on the target — you type into it, installers that prompt / REPLs / sudo / log tails all work. It rides the same sealed transport (so it works off-LAN too) and needs the target to have granted your device the Shell permission. The target shows the 'being controlled' banner the whole time.\n" +
    "• AGENTS ON THE REMOTE MACHINE: flip 'Let agents use remote devices' (Network & reachability) and your agents get a device_exec tool — they can run commands on a paired device for tech support, installing software, and development (no SSH keys; over the encrypted device channel). Off by default; admin/elevation on the target still needs its approval. See docs/REMOTE_DEVICES.md.",
};
