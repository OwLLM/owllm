...
const PAGE_DOCS: Record<string, string> = {
  ...
  agents:
    ...
    "• Browser (🌐 on the canvas header): the agents' SHARED WEB BROWSER — a real Chromium the agents drive with the browser_* tools (open pages, click, type, screenshot; logins persist across runs on disk). The panel shows a live screenshot of the current page, lets you open a URL yourself (agents inherit the session — e.g. pre-log into a site for them), and ⏹ stops the daemon. Closing the panel never stops the browser.\n" +
    "• Notebook (📓 on the canvas header): your scratchpad DURING a run — a freeform Brainstorm pane, a 📋 PLAN pane (the digest drafts a real plan — objective, approach, milestones — 'Apply updated plan' takes it, then edit freely), a NEXT-STEPS list (⚡ Feed sends a step to the running team as a steer, or dispatches it as a new goal when idle; 'Auto-feed' dispatches the next pending step by itself each time a run finishes cleanly — write the roadmap, the team walks it), and a 🪄 Digest agent that turns raw notes into the plan + clear steps (ADDITIVE — it only proposes, you apply/add what you like). Saved per project.\n" +
  ...
  code:
    ...
    "• 📓 NOTEBOOK tab: Brainstorm notes, a 📋 PLAN pane (the digest agent drafts a real plan — objective, approach, milestones — from your brainstorm; 'Apply updated plan' takes it, then edit freely), a NEXT-STEPS list, and the 🪄 Digest agent — pick its model right there (the same dropdown as everywhere; ✕ = back to the default). Feeding a step while the coder works = a ⚡ steer (injected between tool calls on local models); idle = a new turn. 'Auto-feed next step' walks the list by itself after each clean finish.\n" +
