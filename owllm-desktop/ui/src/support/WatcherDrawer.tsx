...
const PAGE_DOCS: Record<string, string> = {
  ...
  agents:
    ...
    "• Notebook (📓 on the canvas header): your scratchpad DURING a run — a freeform Brainstorm pane, a NEXT-STEPS list (⚡ Feed sends a step to the running team as a steer, or dispatches it as a new goal when idle; 'Auto-feed' dispatches the next pending step by itself each time a run finishes cleanly — write the roadmap, the team walks it), and a 🪄 Digest agent that rewrites raw notes into clear, self-contained steps (ADDITIVE — it only proposes new steps, you add the ones you like). Saved per project.\n" +
  ...
  code:
    ...
    "• RIGHT COLUMN — RESIZABLE (drag its left edge; min 300px). Its header: the agent MODE (📋 Plan = Kanban plan/act · ⚡ Auto = act directly · 💬 Chat = discuss only — read-only tools, no edits) which the composer's main button follows; 🖥 Terminal = a real shell in the workspace folder, floating above the app window (this app only); USAGE = VS Code-style quota bars for the account behind the picked model (Claude subscription today; other providers expose no quota).\n" +
    "• Below the header, TWO pages on tabs — ⚡ SUPER USER: PROJECT RULES, the same must/prefer/avoid directives the agentic team uses (auto-seeded with the native best-practice set; add/edit/delete + 'Restore defaults'); they ride EVERY coder turn. If the folder is also an agentic project, rules + notebook are SHARED with the team pages ('shared with team' chip).\n" +
    "• 📓 NOTEBOOK tab: Brainstorm notes + a NEXT-STEPS list + the 🪄 Digest agent that rewrites raw notes into implementable steps — pick its model right there (the same dropdown as everywhere; ✕ = back to the default). Feeding a step while the coder works = a ⚡ steer (injected between tool calls on local models); idle = a new turn. 'Auto-feed next step' walks the list by itself after each clean finish.\n" +
