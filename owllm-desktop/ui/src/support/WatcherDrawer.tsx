...
const PAGE_DOCS: Record<string, string> = {
  ...
  code:
    ...
    "• BOTTOM of the column: USAGE first (VS Code-style quota bars for the account behind the picked model — Claude subscription today; plus the app's own recorded traffic for EVERY provider), then the agent MODE (📋 Plan = Kanban plan/act · ⚡ Auto = act directly · 💬 Chat = discuss only — read-only tools, no edits) which the composer's main button follows, and 🖥 Terminal = a real shell in the workspace folder, floating above the app window (this app only) — DRAG its title bar to move it, — hides it (shell keeps running), ✕ ends it.\n" +
