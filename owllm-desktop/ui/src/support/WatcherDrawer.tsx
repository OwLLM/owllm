...
const PAGE_DOCS: Record<string, string> = {
  ...
  agents:
    ...
    "• Browser (🌐 on the canvas header): the agents' NATIVE web browser — a real OwLLM window (the app's own engine, not Python/Playwright, no external Chromium) the agents drive with the browser_* tools (open pages, click, type, fill forms). Logins persist across runs. The panel has three tabs: BROWSE (open a URL / show the window / autofill / stop — agents inherit whatever you log into), PASSWORDS (a local encrypted vault, DPAPI-backed on Windows, that agents autofill — add logins by hand, passwords never shown), and IMPORT (pull saved logins from Chrome / Edge / Brave / Opera; Firefox coming). Closing the panel never stops the browser.\n" +
